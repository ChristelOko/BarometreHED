/*
  ...
  1. Nouvelles catégories
    - Ajout de la catégorie 'feminine_cycle' aux contraintes des tables feelings et feelings_energia
  
  2. Nouvelles données
    - Ajout de ressentis liés au cycle féminin pour chaque type HD
    - Ajout de ressentis génériques pour le cycle féminin
  
  3. Sécurité
    - Aucune modification des politiques de sécurité
*/
    ...
    BEGIN
      ...
      IF current_constraint NOT LIKE '%feminine_cycle%' THEN
        ...
        ALTER TABLE feelings ADD CONSTRAINT feelings_category_check 
          CHECK (category = ANY (ARRAY['general'::text, 'emotional'::text, 'physical'::text, 'mental'::text, 'digestive'::text, 'somatic'::text, 'energetic'::text, 'feminine_cycle'::text]));
      END IF;
    END;
  END IF;
