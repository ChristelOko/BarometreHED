...
const GeneralCategory = ({ onComplete, onBack, userHdType }: GeneralCategoryProps) => {
  ...
  // Récupérer les ressentis de la catégorie 'general'
  const generalFeelings = getFeelingsByCategory('general', userHdType);
  
  // Séparer les ressentis positifs et négatifs
  const positiveFeelings = generalFeelings.filter(feeling => feeling.isPositive);
  const negativeFeelings = generalFeelings.filter(feeling => !feeling.isPositive);
    ...
    return (
      <div className="max-w-5xl mx-auto pt-6 flex items-center justify-center h-64">
        <div className="text-center">
          <div className="w-10 h-10 border-4 border-[#A87878] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
  ...
  // Si aucun ressenti général n'est disponible, afficher un message
  if (generalFeelings.length === 0) {
    return (
      <div className="max-w-5xl mx-auto pt-6">
        <button 
          onClick={onBack}
          className="md:hidden flex items-center text-primary mb-6 hover:underline"
        >
          <ArrowLeft size={16} className="mr-1" />
          Retour aux catégories
        </button>
        
        <div className="bg-[#A87878]/10 rounded-xl p-8 mb-8 border-2 border-[#A87878]/20">
          <h2 className="font-display text-3xl mb-3 text-[#A87878]">✨ État Général</h2>
          <p className="text-lg text-neutral-dark/90">
            Aucun ressenti général n'est disponible pour votre type HD ({userHdType || 'non défini'}).
          </p>
          <p className="italic text-sm text-[#A87878]/80 mt-2">
        ...
        <div className="flex justify-center">
          <Button 
            variant="primary"
            onClick={onBack}
            className="bg-[#A87878] hover:bg-[#A87878]/90"
          >
            Retour aux catégories
          </Button>
        </div>
    ...
    <div className="max-w-5xl mx-auto pt-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="flex items-center text-primary bg-primary/10 px-3 py-1.5 rounded-full hover:bg-primary/20 transition-colors"
        >
          {showAllDescriptions ? (
            <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        <div className="space-y-4">
          <h3 className="font-display text-xl mb-5 text-[#A87878] flex items-center">
            🌸 Ressentis positifs
          </h3>
          <div className="space-y-4">
              ...
              <div key={index} className="relative">
                <button
                  ...
                  className={`w-full text-left p-5 rounded-xl transition-all flex items-center ${
                    selectedFeelings.includes(feeling.name)
                      ? 'bg-[#A87878]/20 border-2 border-[#A87878]'
                      : 'bg-white border-2 border-transparent hover:border-[#A87878]/30 shadow-sm hover:shadow'
                  }`}
                >
                  <div className={`w-5 h-5 rounded border-2 mr-3 flex items-center justify-center ${
                    selectedFeelings.includes(feeling.name)
                      ? 'border-[#A87878] bg-[#A87878] text-white'
                ...
                {!showAllDescriptions && (
                  <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-neutral-dark/50 hover:text-[#A87878]"
                  >
                    <Info size={18} />
                  </button>
                )}
                {(activeDescription === `positive-${index}` || showAllDescriptions) && (
                  <motion.div
                    ...
                    className="mt-2 p-4 bg-[#A87878]/5 border border-[#A87878]/10 rounded-xl text-sm"
                  >
                    <p className="mb-2">{feeling.description}</p>
                    <p className="text-[#A87878] italic">{feeling.probableOrigin}</p>
                  </motion.div>
                )}
              </div>
            ))}
      ...
      <div className="flex flex-col items-center mt-8">
        <Button 
          ...
          className="bg-[#A87878] hover:bg-[#A87878]/90 px-10"
        ...
        <div className="text-center mt-4">
          <p className="text-sm text-[#A87878]/70 italic">
            ✨ Tes ressentis sont précieux et uniques
          </p>
        </div>
      </div>
    </div>
